...
полученный шаблон рендерится, а переменные, такие как `title`, запоминаются. На
...
стандартная тема Tumblr. Моим основным желанием было, чтобы дизайн не вызывал
...
Windows c Internet Exploer'ом это огромный шрифт. И чем обусловлены такие
странные значения в по умолчанию? Дань истории? В конечном итоге, я остановился
на 14px в качестве размера основного шрифта для блогов с длинными постами и
13px — с короткими.
...
dyno засыпает (покупка второго dyno обойдётся в 34.50$ в месяц), и обработка
...
задуматься о том, что вы выигрываете от того, что принуждаете заходить читателей
на ваш сайт (или наоборот, их теряете).
...
Спустя некоторое время я настрою перенаправление с blog.vlevit.org на
